...
def trace_coords(wire)
  vector = /([RULD])(\d+)/
  wire_coords = [[0,0]]
  for v in wire
    cur_coord = wire_coords.last
    m = v.match(vector)
    dir = m[1]
    len = m[2]

    #puts "#{cur_coord}: #{dir} #{len}"
    for i in 0...len.to_i
      case dir
      when 'R'
      ...
      when 'L'
        new_coord = [cur_coord[0], cur_coord[1]-1]
      when 'U'
        new_coord = [cur_coord[0]+1, cur_coord[1]]
      when 'D'
        new_coord = [cur_coord[0]-1, cur_coord[1]]
      end

      wire_coords << new_coord
      cur_coord = new_coord
    end
    #puts "(#{cur_coord.join(",")})"
  end
...
def step_distance(coord, wire1_coords, wire2_coords)
  return wire1_coords.find_index(coord) + wire2_coords.find_index(coord)
end

v_wire1 = parse_wire(wire1)
v_wire2 = parse_wire(wire2)
wire1_coords = trace_coords(v_wire1)
wire2_coords = trace_coords(v_wire2)
intersections = wire1_coords & wire2_coords
intersections.delete([0,0])

# puts wire1_coords.to_s
# puts wire2_coords.to_s
puts intersections.to_s

distances = []
for i in intersections
  #distance = manhattan_distance(i)
  distance = step_distance(i, wire1_coords, wire2_coords)
  # puts "(#{i.join(',')}): #{distance}"
  distances << distance
end
